...
import (
	...
	sap_api_output_formatter "sap-api-integrations-accounting-document-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) AsyncPostAccountingDocument(
	item *requests.Item,
	accepter []string) {
	wg := &sync.WaitGroup{}
	wg.Add(len(accepter))
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	outputDataItem, err := c.callAccountingDocumentSrvAPIRequirementItem("A_OperationalAcctgDocItemCube", item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callAccountingDocumentSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
